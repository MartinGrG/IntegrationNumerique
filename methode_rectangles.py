...
def methode_rectangle_python(liste, a, b, n):
    ...
    for i in range(n):
        ...
        integrale = integrale + (evaluation(liste, (x[-2]+x[-1])/2) * (x[-1] - x[-2])) #Calcul de l'intégrale avec la méthode des rectangle
    return integrale
...
def methode_rectangle_numpy(liste, a, b, n):
    ...
    integrale = np.sum(evaluation(liste, x2))     #Calul de l'intégrale
    return integrale

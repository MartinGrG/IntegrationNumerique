...
def affichage_simpson(liste, a, b, n):
    ...
    for i in range(0, n, 2):
        x_simpson = np.linspace(x[i], x[i + 2], 100)
        y_simpson = np.polyval(np.polyfit([x[i], x[i + 1], x[i + 2]], [y[i], y[i + 1], y[i + 2]], 2), x_simpson)
        plt.plot(x_simpson, y_simpson, color='orange')
        plt.fill_between(x_simpson, 0, y_simpson, color='orange', alpha=0.3)
